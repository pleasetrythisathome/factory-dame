...
def run(config_path: Path, audio_override: Path | None = None,
    ...
    # Optional brainstem layer (Phase 3 cascade — Lerud 2014).
    has_brainstem = "brainstem" in layers_in_file
    if has_brainstem:
        bt, bamp, bpha, bph, bres, bf = _extract_layer(table, "brainstem")
    ...
    bamp_pad = (_pad_for_window(bamp, half_samples)
                if has_brainstem else None)
    ...
    show_brainstem_row = has_brainstem
    ...
    extra = []
    if show_brainstem_row:
        # Phase 3 brainstem layer. Place ABOVE pitch in row order so
        # the cascade reads top-down: brainstem (harmonic generation)
        # → pitch (fundamental tracking, voice extraction). Same
        # frequency layout as pitch by design (Lerud 2014 tonotopic).
        extra.append(("brainstem", 3))
    if show_motor_row:
    ...
    fig_h = (14 + (2 if show_motor_row else 0) + (3 if show_brainstem_row else 0)
             + (3 if show_w_row else 0))
    ...
    ax_brainstem = None
    if show_brainstem_row:
        ax_brainstem = fig.add_subplot(gs[extra_row_idx, :])
        extra_row_idx += 1
    ax_motor = None
    if show_motor_row:
    ...
    # Brainstem heatmap (Phase 3 cascade — Lerud 2014). Shows the
    # nonlinear layer's state. Harmonics generated by canonical input
    # + integer-ratio coupling are visible as horizontal stripes at
    # multiples of any tonal input.
    im_brainstem = None
    b_line = None
    if show_brainstem_row:
        from matplotlib.colors import (
            LinearSegmentedColormap, Normalize, PowerNorm,
        )
        # Brainstem can saturate at higher amplitudes than pitch under
        # cascade. Use percentile-based vmax with gentle gamma so
        # harmonics at lower amplitudes remain visible alongside
        # saturated fundamentals.
        b_vmax = float(max(np.percentile(bamp, 90), 1e-3))
        init_b = np.zeros((len(bf), window_samples), dtype=np.float32)
        # Distinct cmap: teal-green for brainstem (vs pitch navy,
        # rhythm terracotta, motor brown). Reads as a separate
        # layer immediately.
        cmap_brainstem = LinearSegmentedColormap.from_list(
            "nd_brainstem_ink",
            [_PAPER, "#CFD6C5", "#90A89B", "#48766E", "#1F4A4A", "#0A2424"],
        )
        im_brainstem = ax_brainstem.imshow(
            init_b, aspect="auto", origin="lower",
            extent=(-WINDOW_S / 2, WINDOW_S / 2, 0, len(bf)),
            cmap=cmap_brainstem,
            norm=PowerNorm(gamma=0.35, vmin=0, vmax=b_vmax),
            interpolation="nearest",
        )
        ax_brainstem.set_title(
            f"BRAINSTEM  ·  Lerud 2014 cascade  ·  "
            f"{len(bf)} oscillators  ·  {bf[0]:.0f}–{bf[-1]:.0f} Hz  ·  "
            "harmonics emerge here via canonical input + coupling"
        )
        ax_brainstem.set_ylabel("freq")
        b_ticks = np.linspace(0, len(bf) - 1, 6).astype(int)
        ax_brainstem.set_yticks(b_ticks)
        ax_brainstem.set_yticklabels([f"{bf[i]:.0f}" for i in b_ticks])
        b_line = ax_brainstem.axvline(0, color=_INK, linewidth=2.0)
        b_line.set_path_effects([
    ...
    # Map a voice fundamental (Hz) to a fractional y position on the
    # pitch heatmap (oscillator-index data coordinates). Log-interp
    # against the bank's frequencies; +0.5 centres on the bin row.
    _log_pf = np.log(np.maximum(pf, 1e-9))
    _pf_idx = np.arange(len(pf), dtype=np.float64)

    def _voice_y(freq: float) -> float:
        return float(np.interp(np.log(max(freq, 1e-9)),
                               _log_pf, _pf_idx)) + 0.5
    ...
    def update(_frame):
        ...
        if im_brainstem is not None and bamp_pad is not None:
            im_brainstem.set_data(slice_window(bamp_pad, now))
        ...
        # Voice trails — each extracted voice drawn as a line at its
        # FUNDAMENTAL (center_freq) across the visible time window,
        # coloured by persistent ID, line width ∝ amplitude. This is the
        # honest voice layer over the raw pitch heatmap: one voice = one
        # pitch track you can follow as it persists and moves, passing
        # through the playhead at x=0. (Earlier this drew a tick at every
        # harmonic member off at the future edge, so one voice smeared
        # colour across the whole pitch axis and didn't line up with the
        # playhead — visually implying many pitches where there was one.)
        n_feat = len(voices_per_step)
        k_center = now * features_hz_effective
        half_feat = (WINDOW_S / 2.0) * features_hz_effective
        k_lo = max(0, int(np.floor(k_center - half_feat)))
        k_hi = min(n_feat - 1, int(np.ceil(k_center + half_feat)))
        # Per-voice points keyed by ID, carrying the frame index so we
        # only connect CONSECUTIVE frames — a voice that flickers out and
        # reappears must not be bridged by a long segment (that produced
        # spurious near-vertical streaks).
        trail: dict[int, list[tuple[int, float, float, float]]] = {}
        for k in range(k_lo, k_hi + 1):
            x = k / features_hz_effective - now   # time offset from playhead
            for vox in voices_per_step[k]:
                trail.setdefault(vox["id"], []).append(
                    (k, x, _voice_y(vox["center_freq"]), vox["amp"]))
        voice_segments: list = []
        voice_colors: list = []
        voice_widths: list = []
        for vid, pts in trail.items():
            color = _VOICE_PALETTE[vid % len(_VOICE_PALETTE)]
            for (k0, x0, y0, a0), (k1, x1, y1, _a1) in zip(pts, pts[1:]):
                if k1 - k0 != 1:
                    continue  # gap — don't bridge a silent stretch
                voice_segments.append([(x0, y0), (x1, y1)])
                voice_colors.append(color)
                # amplitude → width; clamp so quiet voices stay visible
                # and loud ones read as bold ribbons.
                voice_widths.append(2.0 + 5.0 * min(1.0, a0 / 0.3))
            # A lone point (voice present a single frame) still deserves a
            # short dash so it doesn't vanish.
            if len(pts) == 1:
                _k, x0, y0, a0 = pts[0]
                voice_segments.append([(x0 - 0.05, y0), (x0 + 0.05, y0)])
                voice_colors.append(color)
                voice_widths.append(2.0 + 5.0 * min(1.0, a0 / 0.3))
        voice_lines_p.set_segments(voice_segments)
        voice_lines_p.set_color(voice_colors)
        voice_lines_p.set_linewidths(voice_widths if voice_widths else [1.0])
        ...
        if im_brainstem is not None:
            artists.append(im_brainstem)
